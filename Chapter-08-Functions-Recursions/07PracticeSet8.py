...
# QUESTION: temperatur covertor c to f and f to c:
# def convt_tem(tem, unit):
#     if unit == 'C':
#         return tem*1.8+32
#     elif unit == 'F':
#         return (tem-32)*5/9

# print(convt_tem(45, 'C'))
...
def pass_strength_checker(password):
    ...
    if not any(char in '!@#$%^&*()_+|}{)(<>/;' for char in password):
        return False
    return True
print(pass_strength_checker('Char3.@high5/'))
print(pass_strength_checker('07072000@Psid'))
...
def is_palindrome(strng):
    new_string = strng.lower().replace(" ", "")
